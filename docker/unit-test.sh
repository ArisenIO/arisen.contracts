...
cd /arisen.contracts/build/tests
...
EXIT_STATUS=$?
[[ "$EXIT_STATUS" == 0 ]] && set -e
mv /arisen.contracts/build/tests/Testing/$(ls /arisen.contracts/build/tests/Testing/ | grep '20' | tail -n 1)/Test.xml /artifacts/Test.xml
