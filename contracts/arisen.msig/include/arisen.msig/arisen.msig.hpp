...
#include <arisen/binary_extension.hpp>
...
namespace arisen {
   /**
    * @defgroup arisenmsig arisen.msig
    * @ingroup arisencontracts
    * arisen.msig contract defines the structures and actions needed to manage the proposals and approvals on blockchain.
    * @{
    */
   class [[arisen::contract("arisen.msig")]] multisig : public contract {
      public:
         ...
         void approve( name proposer, name proposal_name, permission_level level,
                       const arisen::binary_extension<arisen::checksum256>& proposal_hash );
         /**
          * Revoke proposal
          *
         ...
         using propose_action = arisen::action_wrapper<"propose"_n, &multisig::propose>;
         using approve_action = arisen::action_wrapper<"approve"_n, &multisig::approve>;
         using unapprove_action = arisen::action_wrapper<"unapprove"_n, &multisig::unapprove>;
         using cancel_action = arisen::action_wrapper<"cancel"_n, &multisig::cancel>;
         using exec_action = arisen::action_wrapper<"exec"_n, &multisig::exec>;
         using invalidate_action = arisen::action_wrapper<"invalidate"_n, &multisig::invalidate>;

      private:
         struct [[arisen::table]] proposal {
            name                            proposal_name;
         ...
         typedef arisen::multi_index< "proposal"_n, proposal > proposals;

         struct [[arisen::table]] old_approvals_info {
            name                            proposal_name;
         ...
         typedef arisen::multi_index< "approvals"_n, old_approvals_info > old_approvals;
         ...
         struct [[arisen::table]] approvals_info {
            uint8_t                 version = 1;
            name                    proposal_name;
         ...
         typedef arisen::multi_index< "approvals2"_n, approvals_info > approvals;

         struct [[arisen::table]] invalidation {
            name         account;
         ...
         typedef arisen::multi_index< "invals"_n, invalidation > invalidations;
   };
   /** @}*/ // end of @defgroup arisenmsig arisen.msig
} /// namespace arisen
