...
// This header is needed until `is_feature_activated` and `preactivate_feature` are added to `arisen.cdt`
 ...
 * ARISEN Contracts
 *
 * @details The design of the ARISEN blockchain calls for a number of smart contracts that are run at a
 ...
 * managing, and/or using an ARISEN blockchain. They are provided for reference purposes:
 * - arisen.bios
 * - arisen.system
 * - arisen.msig
 * - arisen.wrap
...
namespace arisen {
   ...
   /**
    * @defgroup arisenbios arisen.bios
    * @ingroup arisencontracts
    *
    * arisen.bios is a minimalistic system contract that only supplies the actions that are absolutely
    * critical to bootstrap a chain and nothing more.
    *
    * @{
    */
   class [[arisen::contract("arisen.bios")]] bios : public contract {
      public:
          ...
          */
         [[arisen::action]]
         void reqactivated( const arisen::checksum256& feature_digest );
          ...
          */
         typedef arisen::multi_index< "abihash"_n, abi_hash > abi_hash_table;
   ...
   /** @}*/ // end of @defgroup arisenbios arisen.bios
} /// namespace arisen
