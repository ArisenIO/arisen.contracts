...
#include <arisen/action.hpp>
#include <arisen/contract.hpp>
#include <arisen/crypto.hpp>
#include <arisen/fixed_bytes.hpp>
#include <arisen/ignore.hpp>
#include <arisen/print.hpp>
#include <arisen/privileged.hpp>
#include <arisen/producer_schedule.hpp>

// This header is needed until `is_feature_activiated` and `preactivate_feature` are added to `arisen.cdt`
#include <arisen/../../capi/arisen/crypto.h>
...
namespace arisensystem {
   ...
   struct wait_weight {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      RSNLIB_SERIALIZE( wait_weight, (wait_sec)(weight) )
   };
   ...
   struct authority {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      RSNLIB_SERIALIZE( authority, (threshold)(keys)(accounts)(waits) )
   };
   ...
   struct block_header {
      ...
      std::optional<arisen::producer_schedule>   new_producers;
    ...
    */
   struct [[arisen::table("abihash"), arisen::contract("arisen.system")]] abi_hash {
      name              owner;
      checksum256       hash;
      uint64_t primary_key()const { return owner.value; }

      RSNLIB_SERIALIZE( abi_hash, (owner)(hash) )
   };

   // Method parameters commented out to prevent generation of code that parses input data.
   /**
    * The ARISEN core native contract that governs authorization and contracts' abi.
    *
    * @details
    */
   class [[arisen::contract("arisen.system")]] native : public arisen::contract {
      public:
          ...
          * This is useful because when doing authorization checks, the ARISEN based blockchain starts with the
          * action needed to be authorized (and the contract belonging to), and looks up which permission
          * is needed to pass authorization validation. If a link is set, that permission is used for authoraization
          * validation otherwise then active is the default, with the exception of `arisen.any`.
          * `arisen.any` is an implicit permission which exists on every account; you can link actions to `arisen.any`
         ...
         using newaccount_action = arisen::action_wrapper<"newaccount"_n, &native::newaccount>;
         using updateauth_action = arisen::action_wrapper<"updateauth"_n, &native::updateauth>;
         using deleteauth_action = arisen::action_wrapper<"deleteauth"_n, &native::deleteauth>;
         using linkauth_action = arisen::action_wrapper<"linkauth"_n, &native::linkauth>;
         using unlinkauth_action = arisen::action_wrapper<"unlinkauth"_n, &native::unlinkauth>;
         using canceldelay_action = arisen::action_wrapper<"canceldelay"_n, &native::canceldelay>;
         using setcode_action = arisen::action_wrapper<"setcode"_n, &native::setcode>;
         using setabi_action = arisen::action_wrapper<"setabi"_n, &native::setabi>;
   };
   /** @}*/ // @addtogroup arisensystem
}
