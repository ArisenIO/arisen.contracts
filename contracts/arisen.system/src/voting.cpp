...
namespace arisensystem {
   ...
   using arisen::microseconds;
   using arisen::singleton;

   void system_contract::regproducer( const name& producer, const arisen::public_key& producer_key, const std::string& url, uint16_t location ) {
      check( url.size() < 512, "url too long" );
      check( producer_key != arisen::public_key(), "public key should not be the default value" );
   ...
   void system_contract::voteproducer( const name& voter_name, const name& proxy, const std::vector<name>& producers ) {
      ...
      auto com_itr = _combalance.find( voter_name.value );
      if( com_itr != _combalance.end() && com_itr->com_balance.amount > 0 ) {
         check_voting_requirement( voter_name, "voter holding COM tokens must vote for at least 21 producers or for a proxy" );
      }
   }
