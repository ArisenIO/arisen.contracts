...
namespace arisensystem {
   ...
   :native(s,code,ds),
    ...
    _compool(get_self(), get_self().value),
    _comfunds(get_self(), get_self().value),
    _combalance(get_self(), get_self().value),
    _comorders(get_self(), get_self().value)
   {
      //print( "construct system\n" );
      _gstate  = _global.exists() ? _global.get() : get_default_parameters();
      _gstate2 = _global2.exists() ? _global2.get() : arisen_global_state2{};
      _gstate3 = _global3.exists() ? _global3.get() : arisen_global_state3{};
   }
   ...
   void system_contract::update_ram_supply() {
      auto cbt = arisen::current_block_time();
   ...
   void system_contract::activate( const arisen::checksum256& feature_digest ) {
      require_auth( get_self() );
   ...
   void native::setabi( const name& acnt, const std::vector<char>& abi ) {
      arisen::multi_index< "abihash"_n, abi_hash >  table(get_self(), get_self().value);
      auto itr = table.find( acnt.value );
      if( itr == table.end() ) {
         table.emplace( acnt, [&]( auto& row ) {
            row.owner = acnt;
            row.hash = arisen::sha256(const_cast<char*>(abi.data()), abi.size());
         });
      } else {
   ...
   void system_contract::init( unsigned_int version, const symbol& core ) {
      ...
      auto system_token_supply   = arisen::token::get_supply(token_account, core.code() );
      ...
      open_act.send( com_account, core, get_self() );
   }
