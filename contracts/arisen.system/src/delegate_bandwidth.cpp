...
// Unfortunately, this is needed until CDT fixes the duplicate symbol error with arisen::send_deferred
...
namespace arisensystem {
   ...
   void system_contract::buyrambytes( const name& payer, const name& receiver, uint32_t bytes ) {
      ...
      const int64_t rsn_reserve   = itr->quote.balance.amount;
      const int64_t cost          = exchange_state::get_bancor_input( ram_reserve, rsn_reserve, bytes );
   ...
   void system_contract::buyram( const name& payer, const name& receiver, const asset& quant )
   {
      ...
      if ( fee.amount > 0 ) {
         ...
         channel_to_com( ramfee_account, fee );
      }
  ...
  /**
   ...
   void system_contract::sellram( const name& account, int64_t bytes ) {
      ...
      if ( fee > 0 ) {
         ...
         channel_to_com( ramfee_account, asset(fee, core_symbol() ));
      }
   }
                                   ...
                                   const asset& stake_net_delta, const asset& stake_cpu_delta, bool transfer )
   {
      ...
      if ( stake_account != source_stake_from ) { //for arisen both transfer and refund make no sense
         ...
         if ( need_deferred_trx ) {
            ...
            arisen::cancel_deferred( from.value ); // TODO: Remove this line when replacing deferred trxs is fixed
            out.send( from.value, from, true );
         } else {
            arisen::cancel_deferred( from.value );
         }
