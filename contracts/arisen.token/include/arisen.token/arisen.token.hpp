...
namespace arisen {
   ...
   /**
    * @defgroup arisentoken arisen.token
    * @ingroup arisencontracts
    *
    * arisen.token contract
    *
    * @details arisen.token contract defines the structures and actions that allow users to create, issue, and manage
    * tokens on arisen based blockchains.
    * @{
    */
   class [[arisen::contract("arisen.token")]] token : public contract {
      public:
          ...
          *
          * More information can be read [here](https://github.com/ARISENIO/arisen.contracts/issues/62)
          * and [here](https://github.com/ARISENIO/arisen.contracts/issues/61).
          */
         [[arisen::action]]
         ...
         using create_action = arisen::action_wrapper<"create"_n, &token::create>;
         using issue_action = arisen::action_wrapper<"issue"_n, &token::issue>;
         using retire_action = arisen::action_wrapper<"retire"_n, &token::retire>;
         using transfer_action = arisen::action_wrapper<"transfer"_n, &token::transfer>;
         using open_action = arisen::action_wrapper<"open"_n, &token::open>;
         using close_action = arisen::action_wrapper<"close"_n, &token::close>;
      private:
         struct [[arisen::table]] account {
            asset    balance;

            uint64_t primary_key()const { return balance.symbol.code().raw(); }
         };

         struct [[arisen::table]] currency_stats {
            asset    supply;
            asset    max_supply;
            name     issuer;
         ...
         typedef arisen::multi_index< "accounts"_n, account > accounts;
         typedef arisen::multi_index< "stat"_n, currency_stats > stats;

         void sub_balance( const name& owner, const asset& value );
         void add_balance( const name& owner, const asset& value, const name& ram_payer );
   };
   /** @}*/ // end of @defgroup arisentoken arisen.token
} /// namespace arisen
