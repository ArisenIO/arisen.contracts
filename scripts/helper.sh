...
function aos-version-check() {
  ...
  if $(check-version-numbers $INSTALLED_VERSION_MAJOR $INSTALLED_VERSION_MINOR); then
    ...
    echo "Using ARISEN installation at: $ARISEN_INSTALL_DIR"
    echo "Using ARISEN.CDT installation at: $CDT_INSTALL_DIR"
  else
  ...
  export CMAKE_FRAMEWORK_PATH="${ARISEN_INSTALL_DIR}:${CDT_INSTALL_DIR}:${CMAKE_PREFIX_PATH}"
}