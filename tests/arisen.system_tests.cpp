...
BOOST_FIXTURE_TEST_CASE( buysell, arisen_system_tester ) try {
   ...
   transfer( "arisen", "alice1111111", core_sym::from_string("1000.0000"), "arisen" );
   ...
   transfer( "arisen", "alice1111111", core_sym::from_string("100000000.0000"), "arisen" );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, arisen_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( init_arisen_stake_balance + core_sym::from_string("300.0000"), get_balance( N(arisen.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, arisen_system_tester ) try {
   ...
   transfer( "arisen", "bob111111111", core_sym::from_string("1000.0000"), "arisen" );
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, arisen_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "arisen", "alice1111111", core_sym::from_string("2000.0000"), core_sym::from_string("1000.0000") ) );
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_votepay_share, arisen_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...
BOOST_FIXTURE_TEST_CASE(votepay_share_invariant, arisen_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...
BOOST_FIXTURE_TEST_CASE(votepay_share_proxy, arisen_system_tester, * boost::unit_test::tolerance(1e-5)) try {
...
BOOST_FIXTURE_TEST_CASE(votepay_share_update_order, arisen_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...
BOOST_FIXTURE_TEST_CASE(votepay_transition, arisen_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   auto* tbl = control->db().find<arisen::chain::table_id_object, arisen::chain::by_code_scope_table>(
   ...
   tbl = control->db().find<arisen::chain::table_id_object, arisen::chain::by_code_scope_table>(
                  boost::make_tuple( config::system_account_name,
                                     config::system_account_name,
                                     N(producers2) ) );
   BOOST_REQUIRE( !tbl );
...
BOOST_AUTO_TEST_CASE(votepay_transition2, * boost::unit_test::tolerance(1e-10)) try {
   ...
   auto old_core_from_string = [&]( const std::string& s ) {
      return arisen::chain::asset::from_string(s + " " + old_contract_core_symbol_name);
   };
   ...
   t.set_abi(  config::system_account_name, contracts::util::system_abi_old().data() );
   {
      ...
      t.abi_ser.set_abi(abi, arisen_system_tester::abi_serializer_max_time);
   }
   ...
   auto* tbl = t.control->db().find<arisen::chain::table_id_object, arisen::chain::by_code_scope_table>(
                                    boost::make_tuple( config::system_account_name,
                                                       config::system_account_name,
                                                       N(producers2) ) );
   BOOST_REQUIRE( !tbl );
...
BOOST_FIXTURE_TEST_CASE( buyname, arisen_system_tester ) try {
   ...
   transfer( "arisen", "nofail", core_sym::from_string( "1000.0000" ) );
...
BOOST_FIXTURE_TEST_CASE( multiple_namebids, arisen_system_tester ) try {
   ...
   // alice outbids bob on prefb
   {
      ...
      BOOST_REQUIRE_EQUAL( initial_names_balance + core_sym::from_string("0.1001"), get_balance(N(arisen.names)) );
   }
...
BOOST_FIXTURE_TEST_CASE( namebid_pending_winner, arisen_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( ram_inflation, arisen_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( arisenram_ramusage, arisen_system_tester ) try {
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0000"), get_balance( "alice1111111" ) );
   transfer( "arisen", "alice1111111", core_sym::from_string("1000.0000"), "arisen" );
   BOOST_REQUIRE_EQUAL( success(), stake( "arisen", "alice1111111", core_sym::from_string("200.0000"), core_sym::from_string("100.0000") ) );
   ...
   //remove row
   base_tester::push_action( N(arisen.token), N(close), N(alice1111111), mvo()
                             ( "owner", "alice1111111" )
                             ( "symbol", symbol{CORE_SYM} )
   );
   BOOST_REQUIRE_EQUAL( true, get_row_by_account( N(arisen.token), N(alice1111111), N(accounts), symbol{CORE_SYM}.to_symbol_code() ).empty() );

   auto rlm = control->get_resource_limits_manager();
   auto arisenram_ram_usage = rlm.get_account_ram_usage(N(arisen.ram));
   auto alice_ram_usage = rlm.get_account_ram_usage(N(alice1111111));

   BOOST_REQUIRE_EQUAL( success(), sellram( "alice1111111", 2048 ) );

   //make sure that ram was billed to alice, not to arisen.ram
   BOOST_REQUIRE_EQUAL( true, alice_ram_usage < rlm.get_account_ram_usage(N(alice1111111)) );
   BOOST_REQUIRE_EQUAL( arisenram_ram_usage, rlm.get_account_ram_usage(N(arisen.ram)) );

} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( ram_gift, arisen_system_tester ) try {
   ...
   */
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "arisen", "alice1111111", core_sym::from_string("20.0000"), core_sym::from_string("10.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( com_auth, arisen_system_tester ) try {
   ...
   const asset one_rsn      = core_sym::from_string("1.0000");
   const asset one_com      = asset::from_string("1.0000 COM");
   setup_com_accounts( accounts, init_balance );

   const std::string error_msg("missing authority of aliceaccount");
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(deposit), mvo()("owner", alice)("amount", one_rsn) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(withdraw), mvo()("owner", alice)("amount", one_rsn) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(buycom), mvo()("from", alice)("amount", one_rsn) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg),
                        push_action( bob, N(unstaketocom), mvo()("owner", alice)("receiver", alice)("from_net", one_rsn)("from_cpu", one_rsn) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(sellcom), mvo()("from", alice)("com", one_com) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(cnclcomorder), mvo()("owner", alice) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg),
                        push_action( bob, N(rentcpu), mvo()("from", alice)("receiver", alice)("loan_payment", one_rsn)("loan_fund", one_rsn) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg),
                        push_action( bob, N(rentnet), mvo()("from", alice)("receiver", alice)("loan_payment", one_rsn)("loan_fund", one_rsn) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(fundcpuloan), mvo()("from", alice)("loan_num", 1)("payment", one_rsn) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(fundnetloan), mvo()("from", alice)("loan_num", 1)("payment", one_rsn) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(defcpuloan), mvo()("from", alice)("loan_num", 1)("amount", one_rsn) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(defnetloan), mvo()("from", alice)("loan_num", 1)("amount", one_rsn) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(updatecom), mvo()("owner", alice) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(comexec), mvo()("user", alice)("max", 1) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(consolidate), mvo()("owner", alice) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(mvtosavings), mvo()("owner", alice)("com", one_com) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(mvfrsavings), mvo()("owner", alice)("com", one_com) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(closecom), mvo()("owner", alice) ) );

   BOOST_REQUIRE_EQUAL( error("missing authority of arisen"), push_action( alice, N(setcom), mvo()("balance", one_rsn) ) );

} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( buy_sell_com, arisen_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient funds"), buycom( alice, init_balance + one_unit ) );
   BOOST_REQUIRE_EQUAL( asset::from_string("25000.0000 COM"),  get_buycom_result( alice, core_sym::from_string("2.5000") ) );
   produce_blocks(2);
   produce_block(fc::days(5));
   BOOST_REQUIRE_EQUAL( core_sym::from_string("2.5000"),     get_sellcom_result( alice, asset::from_string("25000.0000 COM") ) );

   BOOST_REQUIRE_EQUAL( success(),                           buycom( alice, core_sym::from_string("13.0000") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("13.0000"),    get_com_vote_stake( alice ) );
   BOOST_REQUIRE_EQUAL( success(),                           buycom( alice, core_sym::from_string("17.0000") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("30.0000"),    get_com_vote_stake( alice ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("970.0000"),   get_com_fund(alice) );
   BOOST_REQUIRE_EQUAL( get_com_balance(alice).get_amount(), ratio * asset::from_string("30.0000 COM").get_amount() );
   auto com_pool = get_com_pool();
   BOOST_REQUIRE_EQUAL( core_sym::from_string("30.0000"),  com_pool["total_lendable"].as<asset>() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("30.0000"),  com_pool["total_unlent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0000"),   com_pool["total_lent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( init_rent,                         com_pool["total_rent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( get_com_balance(alice),            com_pool["total_com"].as<asset>() );

   BOOST_REQUIRE_EQUAL( success(), buycom( bob, core_sym::from_string("75.0000") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("925.0000"), get_com_fund(bob) );
   BOOST_REQUIRE_EQUAL( ratio * asset::from_string("75.0000 COM").get_amount(), get_com_balance(bob).get_amount() );
   com_pool = get_com_pool();
   BOOST_REQUIRE_EQUAL( core_sym::from_string("105.0000"), com_pool["total_lendable"].as<asset>() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("105.0000"), com_pool["total_unlent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0000"),   com_pool["total_lent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( init_rent,                         com_pool["total_rent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( get_com_balance(alice) + get_com_balance(bob), com_pool["total_com"].as<asset>() );

   produce_block( fc::days(6) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("user must first buycom"),        sellcom( carol, asset::from_string("5.0000 COM") ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("asset must be a positive amount of (COM, 4)"),
                        sellcom( bob, core_sym::from_string("55.0000") ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("asset must be a positive amount of (COM, 4)"),
                        sellcom( bob, asset::from_string("-75.0030 COM") ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),    sellcom( bob, asset::from_string("750000.0030 COM") ) );

   auto init_total_com      = com_pool["total_com"].as<asset>().get_amount();
   auto init_total_lendable = com_pool["total_lendable"].as<asset>().get_amount();
   BOOST_REQUIRE_EQUAL( success(),                             sellcom( bob, asset::from_string("550001.0000 COM") ) );
   BOOST_REQUIRE_EQUAL( asset::from_string("199999.0000 COM"), get_com_balance(bob) );
   com_pool = get_com_pool();
   auto total_com      = com_pool["total_com"].as<asset>().get_amount();
   auto total_lendable = com_pool["total_lendable"].as<asset>().get_amount();
   BOOST_REQUIRE_EQUAL( init_total_com / init_total_lendable,          total_com / total_lendable );
   BOOST_REQUIRE_EQUAL( total_lendable,                                com_pool["total_unlent"].as<asset>().get_amount() );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0000"),               com_pool["total_lent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( init_rent,                                     com_pool["total_rent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( get_com_balance(alice) + get_com_balance(bob), com_pool["total_com"].as<asset>() );
...
BOOST_FIXTURE_TEST_CASE( buy_sell_small_com, arisen_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( ratio * payment.get_amount(),               get_buycom_result( alice, payment ).get_amount() );

   produce_blocks(2);
   produce_block( fc::days(5) );
   produce_blocks(2);

   asset init_com_stake = get_com_vote_stake( alice );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("proceeds are negligible"), sellcom( alice, asset::from_string("0.0001 COM") ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("proceeds are negligible"), sellcom( alice, asset::from_string("0.9999 COM") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0001"),            get_sellcom_result( alice, asset::from_string("1.0000 COM") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0001"),            get_sellcom_result( alice, asset::from_string("1.9999 COM") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0002"),            get_sellcom_result( alice, asset::from_string("2.0000 COM") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0002"),            get_sellcom_result( alice, asset::from_string("2.9999 COM") ) );
   BOOST_REQUIRE_EQUAL( get_com_vote_stake( alice ),                init_com_stake - core_sym::from_string("0.0006") );

   BOOST_REQUIRE_EQUAL( success(),                                  rentcpu( carol, bob, core_sym::from_string("10.0000") ) );
   BOOST_REQUIRE_EQUAL( success(),                                  sellcom( alice, asset::from_string("1.0000 COM") ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("proceeds are negligible"), sellcom( alice, asset::from_string("0.4000 COM") ) );

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( unstake_buy_com, arisen_system_tester, * boost::unit_test::tolerance(1e-10) ) try {
   ...
   const std::vector<account_name> accounts = { N(aliceaccount), N(bobbyaccount), N(carolaccount), N(emilyaccount), N(frankaccount) };
   account_name alice = accounts[0], bob = accounts[1], carol = accounts[2], emily = accounts[3], frank = accounts[4];
   setup_com_accounts( accounts, init_balance, init_net, init_cpu, false );
...

   {
      ...
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("must vote for at least 21 producers or for a proxy before buying COM"),
                           unstaketocom( alice, alice, net_stake, cpu_stake ) );
      BOOST_REQUIRE_EQUAL( success(),
                           vote( alice, std::vector<account_name>(producer_names.begin(), producer_names.begin() + 21) ) );
      const asset init_arisen_stake_balance = get_balance( N(arisen.stake) );
      ...
      BOOST_REQUIRE_EQUAL( ratio * tot_stake.get_amount(),          get_unstaketocom_result( alice, alice, net_stake, cpu_stake ).get_amount() );
      BOOST_REQUIRE_EQUAL( get_cpu_limit( alice ),                  init_cpu_limit );
      BOOST_REQUIRE_EQUAL( get_net_limit( alice ),                  init_net_limit );
      BOOST_REQUIRE_EQUAL( ratio * tot_stake.get_amount(),          get_com_balance( alice ).get_amount() );
      BOOST_REQUIRE_EQUAL( tot_stake,                               get_com_balance_obj( alice )["vote_stake"].as<asset>() );
      BOOST_REQUIRE_EQUAL( tot_stake,                               get_balance( N(arisen.com) ) );
      BOOST_REQUIRE_EQUAL( tot_stake,                               init_arisen_stake_balance - get_balance( N(arisen.stake) ) );
...

   {
      ...
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("amount exceeds tokens staked for net"),
                           unstaketocom( bob, carol, net_stake + one_token, zero_asset ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("amount exceeds tokens staked for cpu"),
                           unstaketocom( bob, carol, zero_asset, cpu_stake + one_token ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("delegated bandwidth record does not exist"),
                           unstaketocom( bob, emily, zero_asset, one_token ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("must unstake a positive amount to buy com"),
                           unstaketocom( bob, carol, zero_asset, zero_asset ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("must unstake a positive amount to buy com"),
                           unstaketocom( bob, carol, neg_asset, one_token ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("must unstake a positive amount to buy com"),
                           unstaketocom( bob, carol, one_token, neg_asset ) );
      ...
      BOOST_REQUIRE_EQUAL( success(),                               unstaketocom( bob, carol, net_stake, zero_asset ) );
      BOOST_REQUIRE_EQUAL( false,                                   get_dbw_obj( bob, carol ).is_null() );
      BOOST_REQUIRE_EQUAL( success(),                               unstaketocom( bob, carol, zero_asset, cpu_stake ) );
      BOOST_REQUIRE_EQUAL( true,                                    get_dbw_obj( bob, carol ).is_null() );
      BOOST_REQUIRE_EQUAL( 0,                                       get_com_balance( carol ).get_amount() );
      BOOST_REQUIRE_EQUAL( ratio * tot_stake.get_amount(),          get_com_balance( bob ).get_amount() );
...

   {
      ...
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("delegated bandwidth record does not exist"),
                           unstaketocom( emily, frank, net_stake, cpu_stake ) );
      BOOST_REQUIRE_EQUAL( success(),                               unstaketocom( frank, frank, net_stake, cpu_stake ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( frank, asset::from_string("1.0000 COM") ) );
      produce_block( fc::days(5) );
      BOOST_REQUIRE_EQUAL( success(),                               sellcom( frank, asset::from_string("1.0000 COM") ) );
   }

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( buy_rent_com, arisen_system_tester ) try {
   ...
   const std::vector<account_name> accounts = { N(aliceaccount), N(bobbyaccount), N(carolaccount), N(emilyaccount), N(frankaccount) };
   account_name alice = accounts[0], bob = accounts[1], carol = accounts[2], emily = accounts[3], frank = accounts[4];
   setup_com_accounts( accounts, init_balance, init_net, init_cpu );

   const int64_t init_cpu_limit = get_cpu_limit( alice );
   const int64_t init_net_limit = get_net_limit( alice );

   // bob tries to rent com
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("com system not initialized yet"), rentcpu( bob, carol, core_sym::from_string("5.0000") ) );
   // alice lends com
   BOOST_REQUIRE_EQUAL( success(), buycom( alice, core_sym::from_string("50265.0000") ) );
   BOOST_REQUIRE_EQUAL( init_balance - core_sym::from_string("50265.0000"), get_com_fund(alice) );
   auto com_pool = get_com_pool();
   const asset   init_tot_unlent   = com_pool["total_unlent"].as<asset>();
   const asset   init_tot_lendable = com_pool["total_lendable"].as<asset>();
   const asset   init_tot_rent     = com_pool["total_rent"].as<asset>();
   const int64_t init_stake        = get_voter_info(alice)["staked"].as<int64_t>();
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0000"),        com_pool["total_lent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( ratio * init_tot_lendable.get_amount(), com_pool["total_com"].as<asset>().get_amount() );
   BOOST_REQUIRE_EQUAL( com_pool["total_com"].as<asset>(),      get_com_balance(alice) );

   {
      // bob rents cpu for carol
      const asset fee = core_sym::from_string("17.0000");
      BOOST_REQUIRE_EQUAL( success(),          rentcpu( bob, carol, fee ) );
      BOOST_REQUIRE_EQUAL( init_balance - fee, get_com_fund(bob) );
      com_pool = get_com_pool();
      BOOST_REQUIRE_EQUAL( init_tot_lendable + fee, com_pool["total_lendable"].as<asset>() ); // 65 + 17
      BOOST_REQUIRE_EQUAL( init_tot_rent + fee,     com_pool["total_rent"].as<asset>() );     // 100 + 17
      ...
      BOOST_REQUIRE_EQUAL( expected_total_lent,
                           com_pool["total_lent"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( com_pool["total_lent"].as<asset>() + com_pool["total_unlent"].as<asset>(),
                           com_pool["total_lendable"].as<asset>() );
      ...
      // alice tries to sellcom, order gets scheduled then she cancels order
      BOOST_REQUIRE_EQUAL( cancelcomorder( alice ),           wasm_assert_msg("no sellcom order is scheduled") );
      produce_block( fc::days(6) );
      BOOST_REQUIRE_EQUAL( success(),                         sellcom( alice, get_com_balance(alice) ) );
      BOOST_REQUIRE_EQUAL( success(),                         cancelcomorder( alice ) );
      BOOST_REQUIRE_EQUAL( com_pool["total_com"].as<asset>(), get_com_balance(alice) );
      ...
      // alice is finally able to sellcom, she gains the fee paid by bob
      BOOST_REQUIRE_EQUAL( success(),          sellcom( alice, get_com_balance(alice) ) );
      BOOST_REQUIRE_EQUAL( 0,                  get_com_balance(alice).get_amount() );
      BOOST_REQUIRE_EQUAL( init_balance + fee, get_com_fund(alice) );
      // test that carol's resource limits have been updated properly when loan expires
      BOOST_REQUIRE_EQUAL( init_cpu_limit,     get_cpu_limit( carol ) );
      BOOST_REQUIRE_EQUAL( init_net_limit,     get_net_limit( carol ) );

      com_pool = get_com_pool();
      BOOST_REQUIRE_EQUAL( 0, com_pool["total_lendable"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 0, com_pool["total_unlent"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 0, com_pool["total_com"].as<asset>().get_amount() );
   }

   {
      const int64_t init_net_limit = get_net_limit( emily );
      BOOST_REQUIRE_EQUAL( 0,         get_com_balance(alice).get_amount() );
      BOOST_REQUIRE_EQUAL( success(), buycom( alice, core_sym::from_string("20050.0000") ) );
      com_pool = get_com_pool();
      const asset fee = core_sym::from_string("0.4560");
      int64_t expected_net = bancor_convert( com_pool["total_rent"].as<asset>().get_amount(),
                                             com_pool["total_unlent"].as<asset>().get_amount(),
                                             fee.get_amount() );
...
BOOST_FIXTURE_TEST_CASE( buy_sell_sell_com, arisen_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(),              buycom( alice, payment ) );
   BOOST_REQUIRE_EQUAL( success(),              buycom( bob, core_sym::from_string("0.0005") ) );
   BOOST_REQUIRE_EQUAL( init_balance - payment, get_com_fund(alice) );
   auto com_pool = get_com_pool();
   const asset   init_tot_unlent   = com_pool["total_unlent"].as<asset>();
   const asset   init_tot_lendable = com_pool["total_lendable"].as<asset>();
   const asset   init_tot_rent     = com_pool["total_rent"].as<asset>();
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0000"),        com_pool["total_lent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( ratio * init_tot_lendable.get_amount(), com_pool["total_com"].as<asset>().get_amount() );
   BOOST_REQUIRE_EQUAL( com_pool["total_com"].as<asset>(),      get_com_balance(alice) + get_com_balance(bob) );

   // bob rents cpu for carol
   const asset fee = core_sym::from_string("7.0000");
   BOOST_REQUIRE_EQUAL( success(),               rentcpu( bob, carol, fee ) );
   com_pool = get_com_pool();
   BOOST_REQUIRE_EQUAL( init_tot_lendable + fee, com_pool["total_lendable"].as<asset>() );
   BOOST_REQUIRE_EQUAL( init_tot_rent + fee,     com_pool["total_rent"].as<asset>() );

   produce_block( fc::days(5) );
   produce_blocks(2);
   const asset com_tok = asset::from_string("1.0000 COM");
   BOOST_REQUIRE_EQUAL( success(),                                           sellcom( alice, get_com_balance(alice) - com_tok ) );
   BOOST_REQUIRE_EQUAL( false,                                               get_com_order_obj( alice ).is_null() );
   BOOST_REQUIRE_EQUAL( success(),                                           sellcom( alice, com_tok ) );
   BOOST_REQUIRE_EQUAL( sellcom( alice, com_tok ),                           wasm_assert_msg("insufficient funds for current and scheduled orders") );
   BOOST_REQUIRE_EQUAL( ratio * payment.get_amount() - com_tok.get_amount(), get_com_order( alice )["com_requested"].as<asset>().get_amount() );
   BOOST_REQUIRE_EQUAL( success(),                                           consolidate( alice ) );
   BOOST_REQUIRE_EQUAL( 0,                                                   get_com_balance_obj( alice )["com_maturities"].get_array().size() );

   produce_block( fc::days(26) );
   produce_blocks(2);

   BOOST_REQUIRE_EQUAL( success(),  comexec( alice, 2 ) );
   BOOST_REQUIRE_EQUAL( 0,          get_com_balance( alice ).get_amount() );
   BOOST_REQUIRE_EQUAL( 0,          get_com_balance_obj( alice )["matured_com"].as<int64_t>() );
   const asset init_fund = get_com_fund( alice );
   BOOST_REQUIRE_EQUAL( success(),  updatecom( alice ) );
   BOOST_REQUIRE_EQUAL( 0,          get_com_balance( alice ).get_amount() );
   BOOST_REQUIRE_EQUAL( 0,          get_com_balance_obj( alice )["matured_com"].as<int64_t>() );
   BOOST_REQUIRE      ( init_fund < get_com_fund( alice ) );

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( buy_sell_claim_com, arisen_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), buycom( alice, purchase1) );
   BOOST_REQUIRE_EQUAL( success(), buycom( bob,   purchase2) );
   BOOST_REQUIRE_EQUAL( success(), buycom( carol, purchase3) );

   BOOST_REQUIRE_EQUAL( init_balance - purchase1, get_com_fund(alice) );
   BOOST_REQUIRE_EQUAL( purchase1.get_amount(),   get_voter_info(alice)["staked"].as<int64_t>() - init_stake );

   BOOST_REQUIRE_EQUAL( init_balance - purchase2, get_com_fund(bob) );
   BOOST_REQUIRE_EQUAL( init_balance - purchase3, get_com_fund(carol) );

   auto init_alice_com = get_com_balance(alice);
   auto init_bob_com   = get_com_balance(bob);
   auto init_carol_com = get_com_balance(carol);

   BOOST_REQUIRE_EQUAL( core_sym::from_string("20000.0000"), get_com_pool()["total_rent"].as<asset>() );
   ...
   const auto    init_com_pool        = get_com_pool();
   const int64_t total_lendable       = init_com_pool["total_lendable"].as<asset>().get_amount();
   const int64_t total_com            = init_com_pool["total_com"].as<asset>().get_amount();
   const int64_t init_alice_com_stake = ( arisen::chain::uint128_t(init_alice_com.get_amount()) * total_lendable ) / total_com;

   produce_block( fc::days(5) );

   BOOST_REQUIRE_EQUAL( success(), sellcom( alice, asset( 3 * get_com_balance(alice).get_amount() / 4, symbol(SY(4,COM)) ) ) );

   BOOST_TEST_REQUIRE( within_one( init_alice_com.get_amount() / 4, get_com_balance(alice).get_amount() ) );
   BOOST_TEST_REQUIRE( within_one( init_alice_com_stake / 4,        get_com_vote_stake( alice ).get_amount() ) );
   BOOST_TEST_REQUIRE( within_one( init_alice_com_stake / 4,        get_voter_info(alice)["staked"].as<int64_t>() - init_stake ) );

   produce_block( fc::days(5) );

   init_alice_com = get_com_balance(alice);
   BOOST_REQUIRE_EQUAL( success(), sellcom( bob,   get_com_balance(bob) ) );
   BOOST_REQUIRE_EQUAL( success(), sellcom( carol, get_com_balance(carol) ) );
   BOOST_REQUIRE_EQUAL( success(), sellcom( alice, get_com_balance(alice) ) );

   BOOST_REQUIRE_EQUAL( init_bob_com,   get_com_balance(bob) );
   BOOST_REQUIRE_EQUAL( init_carol_com, get_com_balance(carol) );
   BOOST_REQUIRE_EQUAL( init_alice_com, get_com_balance(alice) );

   // now bob's, carol's and alice's sellcom orders have been queued
   BOOST_REQUIRE_EQUAL( true,           get_com_order(alice)["is_open"].as<bool>() );
   BOOST_REQUIRE_EQUAL( init_alice_com, get_com_order(alice)["com_requested"].as<asset>() );
   BOOST_REQUIRE_EQUAL( 0,              get_com_order(alice)["proceeds"].as<asset>().get_amount() );
   BOOST_REQUIRE_EQUAL( true,           get_com_order(bob)["is_open"].as<bool>() );
   BOOST_REQUIRE_EQUAL( init_bob_com,   get_com_order(bob)["com_requested"].as<asset>() );
   BOOST_REQUIRE_EQUAL( 0,              get_com_order(bob)["proceeds"].as<asset>().get_amount() );
   BOOST_REQUIRE_EQUAL( true,           get_com_order(carol)["is_open"].as<bool>() );
   BOOST_REQUIRE_EQUAL( init_carol_com, get_com_order(carol)["com_requested"].as<asset>() );
   BOOST_REQUIRE_EQUAL( 0,              get_com_order(carol)["proceeds"].as<asset>().get_amount() );
   ...
   // total_unlent to close some sellcom orders. only two are processed, bob's and carol's.
   // alices's order is still open.
   // an action is needed to trigger queue processing
   produce_block( fc::hours(2) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("com loans are currently not available"),
                        rentcpu( frank, frank, core_sym::from_string("0.0001") ) );
   {
      auto trace = base_tester::push_action( config::system_account_name, N(comexec), frank,
                                             mvo()("user", frank)("max", 2) );
      auto output = get_comorder_result( trace );
      BOOST_REQUIRE_EQUAL( output.size(),    1 );
      BOOST_REQUIRE_EQUAL( output[0].first,  bob );
      BOOST_REQUIRE_EQUAL( output[0].second, get_com_order(bob)["proceeds"].as<asset>() );
   }

   {
      ...
      BOOST_REQUIRE_EQUAL( false,          get_com_order(bob)["is_open"].as<bool>() );
      BOOST_REQUIRE_EQUAL( init_bob_com,   get_com_order(bob)["com_requested"].as<asset>() );
      BOOST_REQUIRE      ( 0 <             get_com_order(bob)["proceeds"].as<asset>().get_amount() );
   ...
   {
      auto trace1 = base_tester::push_action( config::system_account_name, N(updatecom), bob, mvo()("owner", bob) );
      auto trace2 = base_tester::push_action( config::system_account_name, N(updatecom), carol, mvo()("owner", carol) );
      BOOST_REQUIRE_EQUAL( 0,              get_com_vote_stake( bob ).get_amount() );
      BOOST_REQUIRE_EQUAL( init_stake,     get_voter_info( bob )["staked"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( 0,              get_com_vote_stake( carol ).get_amount() );
      BOOST_REQUIRE_EQUAL( init_stake,     get_voter_info( carol )["staked"].as<int64_t>() );
      auto output1 = get_comorder_result( trace1 );
      auto output2 = get_comorder_result( trace2 );
      BOOST_REQUIRE_EQUAL( 2,              output1.size() + output2.size() );

      BOOST_REQUIRE_EQUAL( false,          get_com_order_obj(alice).is_null() );
      BOOST_REQUIRE_EQUAL( true,           get_com_order_obj(bob).is_null() );
      BOOST_REQUIRE_EQUAL( true,           get_com_order_obj(carol).is_null() );
      BOOST_REQUIRE_EQUAL( false,          get_com_order(alice)["is_open"].as<bool>() );

      const auto& com_pool = get_com_pool();
      BOOST_REQUIRE_EQUAL( 0,              com_pool["total_lendable"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 0,              com_pool["total_com"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("com loans are currently not available"),
                           rentcpu( frank, frank, core_sym::from_string("1.0000") ) );

      BOOST_REQUIRE_EQUAL( success(),      buycom( emily, core_sym::from_string("22000.0000") ) );
      BOOST_REQUIRE_EQUAL( false,          get_com_order_obj(alice).is_null() );
      BOOST_REQUIRE_EQUAL( false,          get_com_order(alice)["is_open"].as<bool>() );
...
BOOST_FIXTURE_TEST_CASE( com_loans, arisen_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), buycom( alice, core_sym::from_string("25000.0000") ) );
   ...
   asset cur_frank_balance  = get_com_fund( frank );
   int64_t expected_stake   = bancor_convert( com_pool["total_rent"].as<asset>().get_amount(),
                                              com_pool["total_unlent"].as<asset>().get_amount(),
                                              payment.get_amount() );
   ...
   cur_frank_balance      = get_com_fund( frank );
   ...
   com_pool = get_com_pool();
   {
      int64_t unlent_tokens = bancor_convert( com_pool["total_unlent"].as<asset>().get_amount(),
                                              com_pool["total_rent"].as<asset>().get_amount(),
                                              expected_stake );

      expected_stake = bancor_convert( com_pool["total_rent"].as<asset>().get_amount() - unlent_tokens,
                                       com_pool["total_unlent"].as<asset>().get_amount() + expected_stake,
                                       payment.get_amount() );
   }
   ...
   BOOST_REQUIRE_EQUAL( success(),  buycom( alice, core_sym::from_string("10.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( com_loan_checks, arisen_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), buycom( alice, payment1 ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("loan price does not favor renting"),
                        rentcpu( bob, bob, fee ) );
   BOOST_REQUIRE_EQUAL( success(),            buycom( alice, payment2 ) );
   ...
   BOOST_REQUIRE_EQUAL( success(),            comexec( alice, 3) );
   BOOST_REQUIRE_EQUAL( 2 * fee.get_amount(), get_last_cpu_loan()["balance"].as<asset>().get_amount() );

   BOOST_REQUIRE_EQUAL( success(),            sellcom( alice, asset::from_string("1000000.0000 COM") ) );
   produce_block( fc::days(31) );
   BOOST_REQUIRE_EQUAL( success(),            comexec( alice, 3) );
   BOOST_REQUIRE_EQUAL( true,                 get_cpu_loan(1).is_null() );
...
BOOST_FIXTURE_TEST_CASE( ramfee_namebid_to_com, arisen_system_tester ) try {
   ...
   auto cur_com_pool = get_com_pool();

   BOOST_REQUIRE_EQUAL( cur_com_balance, cur_com_pool["total_unlent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( 0,               cur_com_pool["total_lent"].as<asset>().get_amount() );
   BOOST_REQUIRE_EQUAL( cur_com_balance, cur_com_pool["total_lendable"].as<asset>() );
   BOOST_REQUIRE_EQUAL( 0,               cur_com_pool["namebid_proceeds"].as<asset>().get_amount() );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("23.7000"), get_balance( N(arisen.names) ) );
   BOOST_REQUIRE_EQUAL( success(),                        bidname( alice, N(rndmbid), core_sym::from_string("29.3500") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("29.3500"), get_balance( N(arisen.names) ));

   produce_block( fc::hours(24) );
   produce_blocks( 2 );

   BOOST_REQUIRE_EQUAL( core_sym::from_string("29.3500"), get_com_pool()["namebid_proceeds"].as<asset>() );
   BOOST_REQUIRE_EQUAL( success(),                        deposit( frank, core_sym::from_string("5.0000") ) );
   BOOST_REQUIRE_EQUAL( success(),                        buycom( frank, core_sym::from_string("5.0000") ) );
   BOOST_REQUIRE_EQUAL( get_balance( N(arisen.com) ),      cur_com_balance + core_sym::from_string("34.3500") );
   BOOST_REQUIRE_EQUAL( 0,                                get_balance( N(arisen.names) ).get_amount() );

   cur_com_balance = get_balance( N(arisen.com) );
   BOOST_REQUIRE_EQUAL( cur_com_balance,                  get_com_pool()["total_lendable"].as<asset>() );
   BOOST_REQUIRE_EQUAL( cur_com_balance,                  get_com_pool()["total_unlent"].as<asset>() );

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( com_maturity, arisen_system_tester ) try {
   ...
   const symbol  com_sym( SY(4, COM) );

   {
      BOOST_REQUIRE_EQUAL( success(), buycom( alice, core_sym::from_string("11.5000") ) );
      produce_block( fc::hours(3) );
      BOOST_REQUIRE_EQUAL( success(), buycom( alice, core_sym::from_string("18.5000") ) );
      produce_block( fc::hours(25) );
      BOOST_REQUIRE_EQUAL( success(), buycom( alice, core_sym::from_string("25.0000") ) );

      auto com_balance = get_com_balance_obj( alice );
      BOOST_REQUIRE_EQUAL( 550000 * com_ratio, com_balance["com_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 0,                  com_balance["matured_com"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( 2,                  com_balance["com_maturities"].get_array().size() );

      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( alice, asset::from_string("115000.0000 COM") ) );
      produce_block( fc::hours( 3*24 + 20) );
      BOOST_REQUIRE_EQUAL( success(),          sellcom( alice, asset::from_string("300000.0000 COM") ) );
      com_balance = get_com_balance_obj( alice );
      BOOST_REQUIRE_EQUAL( 250000 * com_ratio, com_balance["com_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 0,                  com_balance["matured_com"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( 1,                  com_balance["com_maturities"].get_array().size() );
      produce_block( fc::hours(23) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( alice, asset::from_string("250000.0000 COM") ) );
      produce_block( fc::days(1) );
      BOOST_REQUIRE_EQUAL( success(),          sellcom( alice, asset::from_string("130000.0000 COM") ) );
      com_balance = get_com_balance_obj( alice );
      BOOST_REQUIRE_EQUAL( 1200000000,         com_balance["com_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 1200000000,         com_balance["matured_com"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( 0,                  com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( alice, asset::from_string("130000.0000 COM") ) );
      BOOST_REQUIRE_EQUAL( success(),          sellcom( alice, asset::from_string("120000.0000 COM") ) );
      com_balance = get_com_balance_obj( alice );
      BOOST_REQUIRE_EQUAL( 0,                  com_balance["com_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 0,                  com_balance["matured_com"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( 0,                  com_balance["com_maturities"].get_array().size() );
   }

   {
      ...
      const asset com_bucket( com_ratio * payment.get_amount(), com_sym );
      for ( uint8_t i = 0; i < 8; ++i ) {
         BOOST_REQUIRE_EQUAL( success(), buycom( bob, payment1 ) );
         produce_block( fc::hours(2) );
         BOOST_REQUIRE_EQUAL( success(), buycom( bob, payment2 ) );
         produce_block( fc::hours(24) );
      }

      auto com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 8 * com_bucket.get_amount(), com_balance["com_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 5,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 3 * com_bucket.get_amount(), com_balance["matured_com"].as<int64_t>() );

      BOOST_REQUIRE_EQUAL( success(),                   updatecom( bob ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 4,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 4 * com_bucket.get_amount(), com_balance["matured_com"].as<int64_t>() );
      ...
      produce_block( fc::hours(1) );
      BOOST_REQUIRE_EQUAL( success(),                   sellcom( bob, asset( 3 * com_bucket.get_amount(), com_sym ) ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 4,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( com_bucket.get_amount(),     com_balance["matured_com"].as<int64_t>() );

      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( bob, asset( 2 * com_bucket.get_amount(), com_sym ) ) );
      BOOST_REQUIRE_EQUAL( success(),                   sellcom( bob, asset( com_bucket.get_amount(), com_sym ) ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 4 * com_bucket.get_amount(), com_balance["com_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 4,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 0,                           com_balance["matured_com"].as<int64_t>() );
      ...
      BOOST_REQUIRE_EQUAL( success(),                   updatecom( bob ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 3,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( com_bucket.get_amount(),     com_balance["matured_com"].as<int64_t>() );
      ...
      produce_block( fc::days(3) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( bob, asset( 4 * com_bucket.get_amount(), com_sym ) ) );
      produce_block( fc::hours(24 + 20) );
      BOOST_REQUIRE_EQUAL( success(),                   sellcom( bob, asset( 4 * com_bucket.get_amount(), com_sym ) ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 0,                           com_balance["com_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 0,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 0,                           com_balance["matured_com"].as<int64_t>() );
...

   {
      ...
      const asset com_bucket1( com_ratio * payment1.get_amount(), com_sym );
      const asset com_bucket2( com_ratio * payment2.get_amount(), com_sym );
      const asset tot_com = com_bucket1 + com_bucket2;
      ...
      auto com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( tot_com,                  com_balance["com_balance"].as<asset>() );
      BOOST_REQUIRE_EQUAL( com_bucket1.get_amount(), com_balance["matured_com"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( success(),                rentcpu( alice, alice, core_sym::from_string("8000.0000") ) );
      BOOST_REQUIRE_EQUAL( success(),                sellcom( bob, asset( com_bucket1.get_amount() - 20, com_sym ) ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( com_bucket1.get_amount(), get_com_order( bob )["com_requested"].as<asset>().get_amount() + 20 );
      BOOST_REQUIRE_EQUAL( tot_com,                  com_balance["com_balance"].as<asset>() );
      BOOST_REQUIRE_EQUAL( com_bucket1.get_amount(), com_balance["matured_com"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( success(),                consolidate( bob ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( com_bucket1.get_amount(), com_balance["matured_com"].as<int64_t>() + 20 );
      BOOST_REQUIRE_EQUAL( success(),                cancelcomorder( bob ) );
      BOOST_REQUIRE_EQUAL( success(),                consolidate( bob ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 0,                        com_balance["matured_com"].as<int64_t>() );
   }
...
BOOST_FIXTURE_TEST_CASE( com_savings, arisen_system_tester ) try {
...

   {
      ...
      BOOST_REQUIRE_EQUAL( success(),                   mvtosavings( alice, asset( 8 * com_bucket.get_amount(), com_sym ) ) );
      com_balance = get_com_balance_obj( alice );
      BOOST_REQUIRE_EQUAL( 1,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 0,                           com_balance["matured_com"].as<int64_t>() );
      produce_block( fc::days(1000) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( alice, asset::from_string( "1.0000 COM" ) ) );
      BOOST_REQUIRE_EQUAL( success(),                   mvfrsavings( alice, asset::from_string( "10.0000 COM" ) ) );
      com_balance = get_com_balance_obj( alice );
      BOOST_REQUIRE_EQUAL( 2,                           com_balance["com_maturities"].get_array().size() );
      produce_block( fc::days(3) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
      ...
      produce_block( fc::days(2) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( alice, asset::from_string( "10.0001 COM" ) ) );
      BOOST_REQUIRE_EQUAL( success(),                   sellcom( alice, asset::from_string( "10.0000 COM" ) ) );
      com_balance = get_com_balance_obj( alice );
      BOOST_REQUIRE_EQUAL( 1,                           com_balance["com_maturities"].get_array().size() );
      produce_block( fc::days(100) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( alice, asset::from_string( "0.0001 COM" ) ) );
      BOOST_REQUIRE_EQUAL( success(),                   mvfrsavings( alice, get_com_balance( alice ) ) );
      produce_block( fc::days(5) );
      BOOST_REQUIRE_EQUAL( success(),                   sellcom( alice, get_com_balance( alice ) ) );
   }

   {
      ...
      for ( uint8_t i = 0; i < 5; ++i ) {
         produce_block( fc::hours(24) );
         BOOST_REQUIRE_EQUAL( success(), buycom( bob, payment ) );
      }

      auto com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 5 * com_bucket.get_amount(), com_balance["com_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 5,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 0,                           com_balance["matured_com"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( success(),                   mvtosavings( bob, asset( com_bucket.get_amount() / 2, com_sym ) ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 6,                           com_balance["com_maturities"].get_array().size() );

      BOOST_REQUIRE_EQUAL( success(),                   mvtosavings( bob, asset( com_bucket.get_amount() / 2, com_sym ) ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 5,                           com_balance["com_maturities"].get_array().size() );
      produce_block( fc::days(1) );
      BOOST_REQUIRE_EQUAL( success(),                   sellcom( bob, com_bucket ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 4,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 0,                           com_balance["matured_com"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( 4 * com_bucket.get_amount(), com_balance["com_balance"].as<asset>().get_amount() );

      BOOST_REQUIRE_EQUAL( success(),                   mvtosavings( bob, asset( 3 * com_bucket.get_amount() / 2, com_sym ) ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 3,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( bob, com_bucket ) );
      ...
      BOOST_REQUIRE_EQUAL( 2,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 0,                           com_balance["matured_com"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( 3 * com_bucket.get_amount(), com_balance["com_balance"].as<asset>().get_amount() );

      produce_block( fc::days(1) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( bob, com_bucket ) );
      BOOST_REQUIRE_EQUAL( success(),                   sellcom( bob, asset( com_bucket.get_amount() / 2, com_sym ) ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 1,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 0,                           com_balance["matured_com"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( 5 * com_bucket.get_amount(), 2 * com_balance["com_balance"].as<asset>().get_amount() );

      produce_block( fc::days(20) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( bob, com_bucket ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient COM in savings"),
                           mvfrsavings( bob, asset( 3 * com_bucket.get_amount(), com_sym ) ) );
      BOOST_REQUIRE_EQUAL( success(),                   mvfrsavings( bob, com_bucket ) );
      BOOST_REQUIRE_EQUAL( 2,                           get_com_balance_obj( bob )["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient COM balance"),
                           mvtosavings( bob, asset( 3 * com_bucket.get_amount() / 2, com_sym ) ) );
      produce_block( fc::days(1) );
      BOOST_REQUIRE_EQUAL( success(),                   mvfrsavings( bob, com_bucket ) );
      BOOST_REQUIRE_EQUAL( 3,                           get_com_balance_obj( bob )["com_maturities"].get_array().size() );
      produce_block( fc::days(4) );
      BOOST_REQUIRE_EQUAL( success(),                   sellcom( bob, com_bucket ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( bob, com_bucket ) );
      ...
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 1,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( com_bucket.get_amount() / 2, com_balance["com_balance"].as<asset>().get_amount() );

      BOOST_REQUIRE_EQUAL( success(),                   mvfrsavings( bob, asset( com_bucket.get_amount() / 4, com_sym ) ) );
      produce_block( fc::days(2) );
      BOOST_REQUIRE_EQUAL( success(),                   mvfrsavings( bob, asset( com_bucket.get_amount() / 8, com_sym ) ) );
      BOOST_REQUIRE_EQUAL( 3,                           get_com_balance_obj( bob )["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( success(),                   consolidate( bob ) );
      BOOST_REQUIRE_EQUAL( 2,                           get_com_balance_obj( bob )["com_maturities"].get_array().size() );
      ...
      produce_block( fc::days(5) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient available com"),
                           sellcom( bob, asset( com_bucket.get_amount() / 2, com_sym ) ) );
      BOOST_REQUIRE_EQUAL( success(),                   sellcom( bob, asset( 3 * com_bucket.get_amount() / 8, com_sym ) ) );
      com_balance = get_com_balance_obj( bob );
      BOOST_REQUIRE_EQUAL( 1,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 0,                           com_balance["matured_com"].as<int64_t>() );
      BOOST_REQUIRE_EQUAL( com_bucket.get_amount() / 8, com_balance["com_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( success(),                   mvfrsavings( bob, get_com_balance( bob ) ) );
      produce_block( fc::days(5) );
      BOOST_REQUIRE_EQUAL( success(),                   sellcom( bob, get_com_balance( bob ) ) );
   }
   ...
   {
      const asset payment = core_sym::from_string("40000.0000");
      const int64_t com_bucket_amount = com_ratio * payment.get_amount();
      const asset com_bucket( com_bucket_amount, com_sym );
      BOOST_REQUIRE_EQUAL( success(),  buycom( alice, payment ) );
      BOOST_REQUIRE_EQUAL( com_bucket, get_com_balance( alice ) );
      BOOST_REQUIRE_EQUAL( com_bucket, get_com_pool()["total_com"].as<asset>() );

      produce_block( fc::days(5) );

      BOOST_REQUIRE_EQUAL( success(),                       rentcpu( bob, bob, core_sym::from_string("2000.0000") ) );
      BOOST_REQUIRE_EQUAL( success(),                       sellcom( alice, asset( 9 * com_bucket_amount / 10, com_sym ) ) );
      BOOST_REQUIRE_EQUAL( com_bucket,                      get_com_balance( alice ) );
      BOOST_REQUIRE_EQUAL( success(),                       mvtosavings( alice, asset( com_bucket_amount / 10, com_sym ) ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient COM balance"),
                           mvtosavings( alice, asset( com_bucket_amount / 10, com_sym ) ) );
      BOOST_REQUIRE_EQUAL( success(),                       cancelcomorder( alice ) );
      BOOST_REQUIRE_EQUAL( success(),                       mvtosavings( alice, asset( com_bucket_amount / 10, com_sym ) ) );
      auto rb = get_com_balance_obj( alice );
      BOOST_REQUIRE_EQUAL( rb["matured_com"].as<int64_t>(), 8 * com_bucket_amount / 10 );
      BOOST_REQUIRE_EQUAL( success(),                       mvfrsavings( alice, asset( 2 * com_bucket_amount / 10, com_sym ) ) );
      produce_block( fc::days(31) );
      BOOST_REQUIRE_EQUAL( success(),                       sellcom( alice, get_com_balance( alice ) ) );
   }

   {
      ...
      const int64_t com_bucket_amount      = com_ratio * payment.get_amount();
      const int64_t half_com_bucket_amount = com_bucket_amount / 2;
      const asset   com_bucket( com_bucket_amount, com_sym );
      const asset   half_com_bucket( half_com_bucket_amount, com_sym );

      BOOST_REQUIRE_EQUAL( success(),                   buycom( carol, payment ) );
      BOOST_REQUIRE_EQUAL( com_bucket,                  get_com_balance( carol ) );
      auto com_balance = get_com_balance_obj( carol );

      BOOST_REQUIRE_EQUAL( 1,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 0,                           com_balance["matured_com"].as<int64_t>() );
      produce_block( fc::days(1) );
      BOOST_REQUIRE_EQUAL( success(),                   buycom( carol, payment ) );
      com_balance = get_com_balance_obj( carol );
      BOOST_REQUIRE_EQUAL( 2,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 0,                           com_balance["matured_com"].as<int64_t>() );

      BOOST_REQUIRE_EQUAL( success(),                   mvtosavings( carol, half_com_bucket ) );
      com_balance = get_com_balance_obj( carol );
      BOOST_REQUIRE_EQUAL( 3,                           com_balance["com_maturities"].get_array().size() );

      BOOST_REQUIRE_EQUAL( success(),                   buycom( carol, half_payment ) );
      com_balance = get_com_balance_obj( carol );
      BOOST_REQUIRE_EQUAL( 3,                           com_balance["com_maturities"].get_array().size() );

      produce_block( fc::days(5) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("asset must be a positive amount of (COM, 4)"),
                           mvfrsavings( carol, asset::from_string("0.0000 COM") ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("asset must be a positive amount of (COM, 4)"),
                           mvfrsavings( carol, asset::from_string("1.0000 RND") ) );
      BOOST_REQUIRE_EQUAL( success(),                   mvfrsavings( carol, half_com_bucket ) );
      BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient COM in savings"),
                           mvfrsavings( carol, asset::from_string("0.0001 COM") ) );
      com_balance = get_com_balance_obj( carol );
      BOOST_REQUIRE_EQUAL( 1,                           com_balance["com_maturities"].get_array().size() );
      BOOST_REQUIRE_EQUAL( 5 * half_com_bucket_amount,  com_balance["com_balance"].as<asset>().get_amount() );
      BOOST_REQUIRE_EQUAL( 2 * com_bucket_amount,       com_balance["matured_com"].as<int64_t>() );
      produce_block( fc::days(5) );
      BOOST_REQUIRE_EQUAL( success(),                   sellcom( carol, get_com_balance( carol) ) );
   }
...
BOOST_FIXTURE_TEST_CASE( update_com, arisen_system_tester, * boost::unit_test::tolerance(1e-10) ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(),                              buycom( alice, payment ) );
   BOOST_REQUIRE_EQUAL( payment,                                get_com_vote_stake(alice) );
   BOOST_REQUIRE_EQUAL( get_com_vote_stake(alice).get_amount(), get_voter_info(alice)["staked"].as<int64_t>() - init_stake );

   const asset fee = core_sym::from_string("50.0000");
   BOOST_REQUIRE_EQUAL( success(),                              rentcpu( emily, bob, fee ) );
   BOOST_REQUIRE_EQUAL( success(),                              updatecom( alice ) );
   BOOST_REQUIRE_EQUAL( payment + fee,                          get_com_vote_stake(alice) );
   BOOST_REQUIRE_EQUAL( get_com_vote_stake(alice).get_amount(), get_voter_info( alice )["staked"].as<int64_t>() - init_stake );
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("voter holding COM tokens must vote for at least 21 producers or for a proxy"),
                        vote( alice, std::vector<account_name>(producer_names.begin(), producer_names.begin() + 20) ) );
   BOOST_REQUIRE_EQUAL( success(),
                        vote( alice, std::vector<account_name>(producer_names.begin(), producer_names.begin() + 21) ) );
   ...
   const asset   init_com             = get_com_balance( alice );
   const auto    current_com_pool     = get_com_pool();
   const int64_t total_lendable       = current_com_pool["total_lendable"].as<asset>().get_amount();
   const int64_t total_com            = current_com_pool["total_com"].as<asset>().get_amount();
   const int64_t init_alice_com_stake = ( arisen::chain::uint128_t(init_com.get_amount()) * total_lendable ) / total_com;
   produce_block( fc::days(5) );
   const asset com_sell_amount( get_com_balance(alice).get_amount() / 4, symbol( SY(4,COM) ) );
   BOOST_REQUIRE_EQUAL( success(),                                       sellcom( alice, com_sell_amount ) );
   BOOST_REQUIRE_EQUAL( init_com,                                        get_com_balance( alice ) + com_sell_amount );
   BOOST_REQUIRE_EQUAL( 3 * init_alice_com_stake,                        4 * get_com_vote_stake( alice ).get_amount() );
   BOOST_REQUIRE_EQUAL( get_voter_info( alice )["staked"].as<int64_t>(), init_stake + get_com_vote_stake(alice).get_amount() );
   BOOST_TEST_REQUIRE( stake2votes( asset( get_voter_info( alice )["staked"].as<int64_t>(), symbol{CORE_SYM} ) )
                       == get_producer_info(producer_names[0])["total_votes"].as<double>() );
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must vote for at least 21 producers or for a proxy before buying COM"),
                        buycom( alice, core_sym::from_string("1.0000") ) );

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( update_com_vote, arisen_system_tester, * boost::unit_test::tolerance(1e-10) ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(),                              buycom( alice, purchase ) );
   BOOST_REQUIRE_EQUAL( purchase,                               get_com_pool()["total_lendable"].as<asset>() );
   BOOST_REQUIRE_EQUAL( purchase,                               get_com_vote_stake(alice) );
   BOOST_REQUIRE_EQUAL( get_com_vote_stake(alice).get_amount(), get_voter_info(alice)["staked"].as<int64_t>() - init_stake_amount );
   BOOST_REQUIRE_EQUAL( purchase,                               get_com_pool()["total_lendable"].as<asset>() );
   ...
   const auto curr_com_pool = get_com_pool();
   BOOST_REQUIRE_EQUAL( curr_com_pool["total_lendable"].as<asset>(), init_com_pool["total_lendable"].as<asset>() + rent );
   BOOST_REQUIRE_EQUAL( success(),
                        vote( alice, std::vector<account_name>(producer_names.begin(), producer_names.begin() + 21) ) );
   BOOST_REQUIRE_EQUAL( (purchase + rent).get_amount(),         get_voter_info(alice)["staked"].as<int64_t>() - init_stake_amount );
   BOOST_REQUIRE_EQUAL( purchase + rent,                        get_com_vote_stake(alice) );
   ...
   BOOST_REQUIRE_EQUAL( purchase + rent + rent,                 get_com_vote_stake(alice) );
   ...
   BOOST_REQUIRE_EQUAL( purchase + rent + rent + rent,          get_com_vote_stake(alice) );
...
BOOST_FIXTURE_TEST_CASE( deposit_com_fund, arisen_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0000"),                   get_com_fund( alice ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("must deposit to COM fund first"), withdraw( alice, core_sym::from_string("0.0001") ) );
   ...
   BOOST_REQUIRE_EQUAL( get_com_fund( alice ),                 deposit_quant );
   BOOST_REQUIRE_EQUAL( success(),                             deposit( alice, deposit_quant ) );
   BOOST_REQUIRE_EQUAL( get_com_fund( alice ),                 deposit_quant + deposit_quant );
   BOOST_REQUIRE_EQUAL( get_balance( alice ),                  init_balance - deposit_quant - deposit_quant );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("insufficient funds"), withdraw( alice, get_com_fund( alice ) + core_sym::from_string("0.0001")) );
   BOOST_REQUIRE_EQUAL( success(),                             withdraw( alice, deposit_quant ) );
   BOOST_REQUIRE_EQUAL( get_com_fund( alice ),                 deposit_quant );
   BOOST_REQUIRE_EQUAL( get_balance( alice ),                  init_balance - deposit_quant );
   BOOST_REQUIRE_EQUAL( success(),                             withdraw( alice, get_com_fund( alice ) ) );
   BOOST_REQUIRE_EQUAL( get_com_fund( alice ).get_amount(),    0 );
   BOOST_REQUIRE_EQUAL( get_balance( alice ),                  init_balance );

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( com_lower_bound, arisen_system_tester ) try {
   ...
   const auto com_pool = get_com_pool();
   const int64_t tot_com      = com_pool["total_com"].as<asset>().get_amount();
   const int64_t tot_unlent   = com_pool["total_unlent"].as<asset>().get_amount();
   const int64_t tot_lent     = com_pool["total_lent"].as<asset>().get_amount();
   const int64_t tot_lendable = com_pool["total_lendable"].as<asset>().get_amount();
   double com_per_rsn = double(tot_com) / double(tot_lendable);
   int64_t sell_amount = com_per_rsn * ( tot_unlent - 0.19 * tot_lent );
   produce_block( fc::days(5) );
   BOOST_REQUIRE_EQUAL( success(), sellcom( alice, asset( sell_amount, com_sym ) ) );
   BOOST_REQUIRE_EQUAL( success(), cancelcomorder( alice ) );
   sell_amount = com_per_rsn * ( tot_unlent - 0.2 * tot_lent );
   BOOST_REQUIRE_EQUAL( success(), sellcom( alice, asset( sell_amount, com_sym ) ) );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("no sellcom order is scheduled"),
                        cancelcomorder( alice ) );

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( close_com, arisen_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( true,              !get_com_fund_obj( alice ).is_null() );
   BOOST_REQUIRE_EQUAL( init_balance,      get_com_fund( alice ) );
   BOOST_REQUIRE_EQUAL( success(),         closecom( alice ) );
   ...
   BOOST_REQUIRE_EQUAL( true,              get_com_balance_obj( bob ).is_null() );
   BOOST_REQUIRE_EQUAL( success(),         buycom( bob, init_balance ) );
   BOOST_REQUIRE_EQUAL( true,              !get_com_balance_obj( bob ).is_null() );
   BOOST_REQUIRE_EQUAL( true,              !get_com_fund_obj( bob ).is_null() );
   BOOST_REQUIRE_EQUAL( 0,                 get_com_fund( bob ).get_amount() );
   BOOST_REQUIRE_EQUAL( closecom( bob ),   wasm_assert_msg("account has remaining COM balance, must sell first") );
   produce_block( fc::days(5) );
   BOOST_REQUIRE_EQUAL( success(),         sellcom( bob, get_com_balance( bob ) ) );
   BOOST_REQUIRE_EQUAL( success(),         closecom( bob ) );
   BOOST_REQUIRE_EQUAL( success(),         withdraw( bob, get_com_fund( bob ) ) );
   BOOST_REQUIRE_EQUAL( success(),         closecom( bob ) );
   BOOST_REQUIRE_EQUAL( true,              get_com_balance_obj( bob ).is_null() );
   BOOST_REQUIRE_EQUAL( true,              get_com_fund_obj( bob ).is_null() );
   ...
   BOOST_REQUIRE_EQUAL( success(),         closecom( carol ) );
   BOOST_REQUIRE_EQUAL( true,              !get_com_fund_obj( carol ).is_null() );

   produce_block( fc::days(10) );

   BOOST_REQUIRE_EQUAL( success(),         closecom( carol ) );
   BOOST_REQUIRE_EQUAL( true,              get_com_balance_obj( carol ).is_null() );
   BOOST_REQUIRE_EQUAL( true,              get_com_fund_obj( carol ).is_null() );

   BOOST_REQUIRE_EQUAL( success(),         rentnet( emily, emily, fee ) );
   BOOST_REQUIRE_EQUAL( true,              !get_com_fund_obj( emily ).is_null() );
   BOOST_REQUIRE_EQUAL( success(),         closecom( emily ) );
   BOOST_REQUIRE_EQUAL( true,              !get_com_fund_obj( emily ).is_null() );
   ...
   BOOST_REQUIRE_EQUAL( closecom( bob ),   success() );
   BOOST_REQUIRE      ( 0 <                get_com_fund( bob ).get_amount() );
   BOOST_REQUIRE_EQUAL( success(),         withdraw( bob, get_com_fund( bob ) ) );
   BOOST_REQUIRE_EQUAL( success(),         closecom( bob ) );
   BOOST_REQUIRE_EQUAL( true,              get_com_balance_obj( bob ).is_null() );
   BOOST_REQUIRE_EQUAL( true,              get_com_fund_obj( bob ).is_null() );

   BOOST_REQUIRE_EQUAL( 0,                 get_com_pool()["total_com"].as<asset>().get_amount() );
   BOOST_REQUIRE_EQUAL( 0,                 get_com_pool()["total_lendable"].as<asset>().get_amount() );
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("com loans are currently not available"),
...
BOOST_FIXTURE_TEST_CASE( set_com, arisen_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg("com system is not initialized"),
                        push_action( config::system_account_name, act_name, mvo()("balance", set_total_rent) ) );
   BOOST_REQUIRE_EQUAL( success(), buycom( alice, init_balance ) );
   ...
   BOOST_REQUIRE_EQUAL( success(),             rentcpu( bob, bob, fee ) );
   const auto& init_com_pool = get_com_pool();
   BOOST_REQUIRE_EQUAL( init_total_rent + fee, init_com_pool["total_rent"].as<asset>() );
   BOOST_TEST_REQUIRE( set_total_rent != init_com_pool["total_rent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( success(),
                        push_action( config::system_account_name, act_name, mvo()("balance", set_total_rent) ) );
   const auto& curr_com_pool = get_com_pool();
   BOOST_REQUIRE_EQUAL( init_com_pool["total_lendable"].as<asset>(),   curr_com_pool["total_lendable"].as<asset>() );
   BOOST_REQUIRE_EQUAL( init_com_pool["total_lent"].as<asset>(),       curr_com_pool["total_lent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( init_com_pool["total_unlent"].as<asset>(),     curr_com_pool["total_unlent"].as<asset>() );
   BOOST_REQUIRE_EQUAL( init_com_pool["namebid_proceeds"].as<asset>(), curr_com_pool["namebid_proceeds"].as<asset>() );
   BOOST_REQUIRE_EQUAL( init_com_pool["loan_num"].as_uint64(),         curr_com_pool["loan_num"].as_uint64() );
   BOOST_REQUIRE_EQUAL( set_total_rent,                                curr_com_pool["total_rent"].as<asset>() );

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( b1_vesting, arisen_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), unstaketocom( b1, b1, half_stake, half_stake ) );

   produce_block( fc::days(5) );
   produce_blocks(1);

   BOOST_REQUIRE_EQUAL( wasm_assert_msg("b1 can only claim their tokens over 10 years"),
                        sellcom( b1, get_com_balance( b1 ) ) );

   produce_block( fc::days( 2 * 364 ) );

   BOOST_REQUIRE_EQUAL( success(), rentcpu( bob, bob, core_sym::from_string("10000.0000") ) );

   BOOST_REQUIRE_EQUAL( wasm_assert_msg("b1 sellcom orders should not be queued"),
                        sellcom( b1, get_com_balance( b1 ) ) );

   produce_block( fc::days( 30 ) );

   BOOST_REQUIRE_EQUAL( success(), sellcom( b1, get_com_balance( b1 ) ) );
...
BOOST_AUTO_TEST_CASE( setabi_bios ) try {
   ...
   t.create_account(N(arisen.token));
   t.set_abi( N(arisen.token), contracts::token_abi().data() );
   {
      auto res = t.get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(arisen.token) );
      _abi_hash abi_hash;
   ...
   t.set_abi( N(arisen.token), contracts::system_abi().data() );
   {
      auto res = t.get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(arisen.token) );
      _abi_hash abi_hash;
      auto abi_hash_var = abi_ser.binary_to_variant( "abi_hash", res, base_tester::abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( setabi, arisen_system_tester ) try {
   set_abi( N(arisen.token), contracts::token_abi().data() );
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(arisen.token) );
      _abi_hash abi_hash;
   ...
   set_abi( N(arisen.token), contracts::system_abi().data() );
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(arisen.token) );
      _abi_hash abi_hash;
      auto abi_hash_var = abi_ser.binary_to_variant( "abi_hash", res, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( change_limited_account_back_to_unlimited, arisen_system_tester ) try {
   BOOST_REQUIRE( get_total_stake( "arisen" ).is_null() );

   transfer( N(arisen), N(alice1111111), core_sym::from_string("1.0000") );

   auto error_msg = stake( N(alice1111111), N(arisen), core_sym::from_string("0.0000"), core_sym::from_string("1.0000") );
   auto semicolon_pos = error_msg.find(';');

   BOOST_REQUIRE_EQUAL( error("account arisen has insufficient ram"),
   ...
   stake( N(alice1111111), N(arisen), core_sym::from_string("0.0000"), core_sym::from_string("1.0000") );

   REQUIRE_MATCHING_OBJECT( get_total_stake( "arisen" ), mvo()
      ("owner", "arisen")
   ...
   BOOST_REQUIRE_EQUAL( success(),
                        push_action( N(arisen), N(setacctnet), mvo()
                           ("account", "arisen")
                           ("net_weight", -1)
                        )
   );
   ...
   BOOST_REQUIRE_EQUAL( success(),
                        push_action( N(arisen), N(setacctcpu), mvo()
                           ("account", "arisen")
                           ("cpu_weight", -1)
...
BOOST_FIXTURE_TEST_CASE( buy_pin_sell_ram, arisen_system_tester ) try {
   BOOST_REQUIRE( get_total_stake( "arisen" ).is_null() );

   transfer( N(arisen), N(alice1111111), core_sym::from_string("1020.0000") );

   auto error_msg = stake( N(alice1111111), N(arisen), core_sym::from_string("10.0000"), core_sym::from_string("10.0000") );
   auto semicolon_pos = error_msg.find(';');
   ...
   BOOST_REQUIRE_EQUAL( success(), buyrambytes( N(alice1111111), N(arisen), static_cast<uint32_t>(ram_bytes_needed) ) );

   auto tokens_paid_for_ram = alice_original_balance - get_balance( N(alice1111111) );

   auto total_res = get_total_stake( "arisen" );
   ...
   BOOST_REQUIRE_EQUAL( success(),
                        push_action( N(arisen), N(setacctram), mvo()
                           ("account", "arisen")
                           ("ram_bytes", total_res["ram_bytes"].as_int64() )
                        )
   );

   auto arisen_original_balance = get_balance( N(arisen) );

   BOOST_REQUIRE_EQUAL( success(), sellram( N(arisen), total_res["ram_bytes"].as_int64() ) );

   auto tokens_received_by_selling_ram = get_balance( N(arisen) ) - arisen_original_balance;
