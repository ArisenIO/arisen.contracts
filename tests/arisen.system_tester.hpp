...
public:
   ...
   void create_core_token( symbol core_symbol = symbol{CORE_SYM} ) {
      ...
      create_currency( N(arisen.token), config::system_account_name, asset(100000000000000, core_symbol) );
      issue( asset(10000000000000, core_symbol) );
      BOOST_REQUIRE_EQUAL( asset(10000000000000, core_symbol), get_balance( "arisen", core_symbol ) );
   }
   ...
   arisen_system_tester( setup_level l = setup_level::full ) {
   ...
   action_result buycom( const account_name& from, const asset& amount ) {
      return push_action( name(from), N(buycom), mvo()
                          ("from",   from)
                          ("amount", amount)
      );
   }

   asset get_buycom_result( const account_name& from, const asset& amount ) {
      auto trace = base_tester::push_action( config::system_account_name, N(buycom), from, mvo()("from", from)("amount", amount) );
      asset com_received;
   ...
   action_result unstaketocom( const account_name& owner, const account_name& receiver, const asset& from_net, const asset& from_cpu ) {
      return push_action( name(owner), N(unstaketocom), mvo()
                          ("owner",    owner)
                          ("receiver", receiver)
                          ("from_net", from_net)
                          ("from_cpu", from_cpu)
   ...
   asset get_unstaketocom_result( const account_name& owner, const account_name& receiver, const asset& from_net, const asset& from_cpu ) {
      auto trace = base_tester::push_action( config::system_account_name, N(unstaketocom), owner, mvo()
                                             ("owner", owner)
                                             ("receiver", receiver)
                                             ("from_net", from_net)
                                             ("from_cpu", from_cpu)
   ...
   action_result sellcom( const account_name& from, const asset& com ) {
      return push_action( name(from), N(sellcom), mvo()
                          ("from", from)
                          ("com",  com)
      );
   }

   asset get_sellcom_result( const account_name& from, const asset& com ) {
      auto trace = base_tester::push_action( config::system_account_name, N(sellcom), from, mvo()("from", from)("com", com) );
      asset proceeds;
      for ( size_t i = 0; i < trace->action_traces.size(); ++i ) {
   ...
   auto get_comorder_result( const transaction_trace_ptr& trace ) {
   ...
   action_result cancelcomorder( const account_name& owner ) {
      return push_action( name(owner), N(cnclcomorder), mvo()("owner", owner) );
   }
   ...
   asset _get_rentcom_result( const account_name& from, const account_name& receiver, const asset& payment, bool cpu ) {
   ...
   asset get_rentcpu_result( const account_name& from, const account_name& receiver, const asset& payment ) {
      return _get_rentcom_result( from, receiver, payment, true );
   }

   asset get_rentnet_result( const account_name& from, const account_name& receiver, const asset& payment ) {
      return _get_rentcom_result( from, receiver, payment, false );
   }
   ...
   action_result updatecom( const account_name& owner ) {
      return push_action( name(owner), N(updatecom), mvo()("owner", owner) );
   }

   action_result comexec( const account_name& user, uint16_t max ) {
      return push_action( name(user), N(comexec), mvo()("user", user)("max", max) );
   }
   ...
   action_result mvtosavings( const account_name& owner, const asset& com ) {
      return push_action( name(owner), N(mvtosavings), mvo()("owner", owner)("com", com) );
   }

   action_result mvfrsavings( const account_name& owner, const asset& com ) {
      return push_action( name(owner), N(mvfrsavings), mvo()("owner", owner)("com", com) );
   }

   action_result closecom( const account_name& owner ) {
      return push_action( name(owner), N(closecom), mvo()("owner", owner) );
   }

   fc::variant get_last_loan(bool cpu) {
      vector<char> data;
      const auto& db = control->db();
      namespace chain = arisen::chain;
      auto table = cpu ? N(cpuloan) : N(netloan);
      const auto* t_id = db.find<arisen::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, table ) );
      if ( !t_id ) {
         return fc::variant();
      }
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "com_loan", data, abi_serializer_max_time );
   }
   ...
   asset get_com_balance( const account_name& act ) const {
      vector<char> data = get_row_by_account( config::system_account_name, config::system_account_name, N(combal), act );
      return data.empty() ? asset(0, symbol(SY(4, COM))) : abi_ser.binary_to_variant("com_balance", data, abi_serializer_max_time)["com_balance"].as<asset>();
   }

   fc::variant get_com_balance_obj( const account_name& act ) const {
      vector<char> data = get_row_by_account( config::system_account_name, config::system_account_name, N(combal), act );
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant("com_balance", data, abi_serializer_max_time);
   }

   asset get_com_fund( const account_name& act ) const {
      vector<char> data = get_row_by_account( config::system_account_name, config::system_account_name, N(comfund), act );
      return data.empty() ? asset(0, symbol{CORE_SYM}) : abi_ser.binary_to_variant("com_fund", data, abi_serializer_max_time)["balance"].as<asset>();
   }

   fc::variant get_com_fund_obj( const account_name& act ) const {
      vector<char> data = get_row_by_account( config::system_account_name, config::system_account_name, N(comfund), act );
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "com_fund", data, abi_serializer_max_time );
   }

   asset get_com_vote_stake( const account_name& act ) const {
      vector<char> data = get_row_by_account( config::system_account_name, config::system_account_name, N(combal), act );
      return data.empty() ? core_sym::from_string("0.0000") : abi_ser.binary_to_variant("com_balance", data, abi_serializer_max_time)["vote_stake"].as<asset>();
   }

   fc::variant get_com_order( const account_name& act ) {
      vector<char> data = get_row_by_account( config::system_account_name, config::system_account_name, N(comqueue), act );
      return abi_ser.binary_to_variant( "com_order", data, abi_serializer_max_time );
   }

   fc::variant get_com_order_obj( const account_name& act ) {
      vector<char> data = get_row_by_account( config::system_account_name, config::system_account_name, N(comqueue), act );
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "com_order", data, abi_serializer_max_time );
   }

   fc::variant get_com_pool() const {
      vector<char> data;
      const auto& db = control->db();
      namespace chain = arisen::chain;
      const auto* t_id = db.find<arisen::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, N(compool) ) );
      if ( !t_id ) {
         return fc::variant();
      }
      ...
      data.resize( itr->value.size() );
      memcpy( data.data(), itr->value.data(), data.size() );
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "com_pool", data, abi_serializer_max_time );
   }

   void setup_com_accounts( const std::vector<account_name>& accounts,
                            const asset& init_balance,
                            const asset& net = core_sym::from_string("80.0000"),
                            const asset& cpu = core_sym::from_string("80.0000"),
                            bool deposit_into_com_fund = true ) {
      ...
      for (const auto& a: accounts) {
         ...
         BOOST_REQUIRE_EQUAL( asset::from_string("0.0000 COM"), get_com_balance(a) );
         if (deposit_into_com_fund) {
            BOOST_REQUIRE_EQUAL( success(),    deposit( a, init_balance ) );
            BOOST_REQUIRE_EQUAL( init_balance, get_com_fund( a ) );
   ...
   asset get_balance( const account_name& act, symbol balance_symbol = symbol{CORE_SYM} ) {
      vector<char> data = get_row_by_account( N(arisen.token), act, N(accounts), balance_symbol.to_symbol_code().value );
   ...
   void issue_and_transfer( const name& to, const asset& amount, const name& manager = config::system_account_name ) {
      signed_transaction trx;
      trx.actions.emplace_back( get_action( N(arisen.token), N(issue),
      ...
      if ( to != manager ) {
         trx.actions.emplace_back( get_action( N(arisen.token), N(transfer),
                                               vector<permission_level>{{manager, config::active_name}},
                                               mutable_variant_object()
                                               ("from",     manager)
                                               ("to",       to )
   ...
   fc::variant get_global_state2() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "arisen_global_state2", data, abi_serializer_max_time );
   }
   ...
   fc::variant get_global_state3() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "arisen_global_state3", data, abi_serializer_max_time );
   }
   ...
   abi_serializer initialize_multisig() {
      abi_serializer msig_abi_ser;
      {
         ...
         BOOST_REQUIRE_EQUAL( success(), buyram( "arisen", "arisen.msig", core_sym::from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "arisen", "alice1111111", core_sym::from_string("650000000.0000"), "arisen" );
...
inline uint64_t M( const string& rsn_str ) {
   return core_sym::from_string( rsn_str ).get_amount();
}
